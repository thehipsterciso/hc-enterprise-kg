...
class TestQueryBuilder:
    ...
    def test_query_with_filter(self, engine):
        engine.add_entity(
            Person(
                id="p1", first_name="A", last_name="B", name="A B", email="a@b.com", is_active=True
            )
        )
        engine.add_entity(
            Person(
                id="p2", first_name="C", last_name="D", name="C D", email="c@d.com", is_active=False
            )
        )
    ...
    def test_query_connected_to(self, engine):
        ...
        engine.add_entity(
            Person(id="p2", first_name="C", last_name="D", name="C D", email="c@d.com")
        )
        engine.add_entity(Department(id="d1", name="Eng"))
