...
The MCP server (`src/mcp_server/`) provides 16 tools for Claude Desktop:
...
- **Write (relationships):** `add_relationship_tool`, `add_relationships_batch` (atomic, max 500), `remove_relationship_tool`
- **Write (entities):** `add_entity_tool`, `update_entity_tool`, `remove_entity_tool`
