...
def healthcare_org(employee_count: int = 2000) -> OrgProfile:
    ...
    return OrgProfile(
        ...
        location_count=max(1, min(100, employee_count // 200 + 1)),
        system_count_range=s(c.systems, 30, 3000),
        network_specs=[
        ...
        vendor_count_range=s(c.vendors, 10, 2000),
        data_asset_count_range=s(c.data_assets, 20, 6000),
        policy_count_range=s(c.policies, 8, 500),
        ...
        regulation_count_range=s(80, 4, 50),
        control_count_range=s(c.controls, 8, 900),
        risk_count_range=s(c.risks, 4, 400),
        threat_count_range=s(c.threats, 2, 80),
        integration_count_range=s(c.integrations, 5, 900),
        data_domain_count_range=s(c.data_domains, 3, 50),
        data_flow_count_range=s(c.data_flows, 5, 2000),
        org_unit_count_range=s(c.org_units, 4, 200),
        capability_count_range=s(c.capabilities, 5, 150),
        site_count_range=s(c.sites, 2, 100),
        geography_count_range=s(c.geographies, 2, 20),
        jurisdiction_count_range=s(c.jurisdictions, 2, 20),
        product_portfolio_count_range=s(c.product_portfolios, 1, 15),
        product_count_range=s(c.products, 4, 150),
